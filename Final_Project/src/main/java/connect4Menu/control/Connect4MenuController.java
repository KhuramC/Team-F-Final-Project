...
/**
 * A Controller from the MVC architecture for the Connect4 menus to play a game of Connect4.
 * @author Khuram C.
 */
public class Connect4MenuController implements MenuController {
	...
	/**
	 * A subclass that listens to the timerToggleButton, gets whether the timer is being used or not,
	 * and adjusts the visibility of the timer settings accordingly.
	 * @author Khuram C.
	 */
	public class TimerToggleButtonListener implements ActionListener{
		...
		/**
		 * Official feature method
		 * Main feature method. Turns the timer on/off and changes the visibility of the timer.
	...
	/**
	 * A subclass that listens to the timerSlider, gets the timer time chosen from it, and sets the timerTextField value to it.
	 * @author Khuram C.
	 */
	...
	/**
	 * A subclass that listens to the timerTextField, gets the timer time chosen from it, and sets the timerSlider value to it.
	 * Exceptions can occur if a non-integer value is passed, or if a integer not within the values is passed.
	 * @author Khuram C.
	 */
	public class TimerTextFieldListener implements ActionListener{
		//Note: this never directly changes the timer value within the model. Since we change the slider,
		//the TimerSliderListener picks it up having been updated to the new value, and sets the timer time itself.
		
		@Override
		public void actionPerformed(ActionEvent e) {
			try {
				int timerTextFieldValue = Integer.parseInt(settingsView.getTimerTextFieldValue());
				if(timerTextFieldValue > Connect4MenuModel.maxTimerTime || timerTextFieldValue < Connect4MenuModel.minTimerTime) {
				...
				settingsView.changeErrorLabelText("Put in an integer from " + Integer.toString(Connect4MenuModel.minTimerTime) 
				+ " to " + Integer.toString(Connect4MenuModel.maxTimerTime) + "s.");	
			}
		}
	}
	
	/**
	 * A subclass that listens to the player1ColorsComboBox and changes the color for Player 1.
	 * @author Khuram C.
	 */
	...
	/**
	 * A subclass that listens to the player2ColorsComboBox and changes the color for Player 2.
	 * @author Khuram C.
	 */
	...
	/**
	 * A subclass that listens to the startGameButton, closes the settingsView, and opens the gameView.
	 * @author Khuram C.
	 */
	...
	/**
	 * A subclass that listens to the RadioButtons and changes the size of the board based on the selection.
	 * @author Khuram C.
	 */
